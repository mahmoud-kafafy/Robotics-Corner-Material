...
main()
 ...
 ├──> Calculate(nums, lambda_square, lambda_cube)
 │       │
 │       ├──> lambda_cube(num)   // for odd numbers
 │       └──> lambda_square(num) // for even numbers
 │
 ├──> Send()
 │       │
 │       ├──> send_data()   // returns true
 │       │
 │       └──> onEvent(response)
 │               │
 │               └──> CommStackOnEvent(onEvent)
 │                          │
 │                          └──> prints "Handling error"
 │
 └──> Send2(send_data)
         │
         ├──> send_data()   // returns false
         │
         └──> onEvent(response)
                 │
                 └──> CommStackOnEvent(onEvent)
                            │
                            └──> prints "Handling request"



                            
Each function delegates work to another:

main controls high-level logic (which feature to run).

Calculate, Send, and Send2 are middle-level.

Lambdas (lambda_square, lambda_cube, send_data, onEvent) are the lowest-level workers.

CommStackOnEvent is like a dispatcher — it takes a callback and executes it.
